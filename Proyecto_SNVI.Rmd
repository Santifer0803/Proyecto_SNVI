...
# 1. Obtener probabilidades predichas (para clase positiva)
probabilidades <- predict(modelo, newdata = data.test, type = "prob")
prob_pos <- probabilidades[, "Incendio"]

# 2. Definir clases reales
clases_reales <- factor(data.test$Classes, levels = c("No.incendio", "Incendio"))

# 3. Calcular curva ROC
roc_rf <- roc(response = clases_reales,
              predictor = prob_pos,
              levels = c("No.incendio", "Incendio"))  # Clase positiva es "Incendio"
...
abline(a = 0, b = 1, col = "gray", lty = 2)  # Línea aleatoria

# 5. Mostrar el valor del AUC
auc_valor <- auc(roc_rf)
legend("bottomright", legend = paste("AUC =", round(auc_valor, 4)), bty = "n")
