...
# Se grafica el ROC
plot(roc.xgb, col = "#F8766D", lwd = 2, legacy.axes = TRUE)
...
mat.conf.xgb <- confusionMatrix(prediccion.xgb, data.test$Classes)

# Se pasa a data.frame
mat.conf.xgb <- as.data.frame(mat.conf.xgb$table)

# Se cambia a factor
mat.conf.xgb$Reference <- factor(mat.conf.xgb$Reference, levels = levels(data.test$Classes))
mat.conf.xgb$Prediction <- factor(mat.conf.xgb$Prediction, levels = c("No.incendio", "Incendio"))
