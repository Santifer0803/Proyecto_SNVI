...
Lo primero es la construcción del modelo, note que ya están creadas los datos de entrenamiento y los datos de test. Con esto se procede con el cálculo del modelo, utilizando el siguiente código.
...
```{r validación cruzada Random Forest}
set.seed(0609) # Fijamos la semilla

control <- trainControl(
  method = "cv", # Método de Validación Cruzada (Cross Validation)
...
Según los resultados anteriores, el ROC más grande se obtiene cuando se toma como hiperparámetro la cantidad de variables aleatorias 7, es decir, el mtry = 7, eso significa que en cada árbol toma como variables aleatorias para clasificar 7 variables. (Un mtry muy grande provoca que todos los árboles se parezcan entre sí, sobre ajusta el modelo. Un mtry muy pequeño no sobreajusta, pero pierde precisión.) 
...
Luego, el valor de Spec, es el valor de especificidad es la cantidad de no éxitos clasificados como no éxitos correctamente. 

Se presenta ahora el valor de la pérdida logarítimica:

```{r pérdida logaritmica - Random Forest}
probabilidades <- predict(modelo, newdata = data.test, type = "prob")


prob_pos <- probabilidades[, "Incendio"]  

y_true <- ifelse(data.test$Classes == "Incendio", 1, 0)

epsilon <- 1e-15
prob_pos <- pmin(pmax(prob_pos, epsilon), 1 - epsilon)

log_loss <- -mean(y_true * log(prob_pos) + (1 - y_true) * log(1 - prob_pos))
print(log_loss)
```
Un valor cercano a 0 de la pérdida logarítmica implica que el modelo está realizando una buena clasificación. 

Se procede con un gráfico de la importancia de la variables a la hora de predecir. Se presentan los gráficos tanto usado por la librería Random Forest, como la librería Caret. Primero con la librería Random Forest.


```{r importancia con Random Forest}
imp <- importance(modelo)  # modelo es tu randomForest

# Transformar a data.frame para ggplot
df_imp <- data.frame(
...
ggplot(df_imp, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  coord_flip() +
  labs(title = "Importancia de Variables (MeanDecreaseGini)", x = "Variable", y = "Importancia") +
  theme_minimal()
```
...
conf_mat_plot <- conf_mat(df_pred, truth, prediction)

autoplot(conf_mat_plot, type = "heatmap") +
...
```{r curva ROC - Random Forest}
roc_rf <- roc(
  response = data.test$Classes,
  predictor = prob_rf$Incendio,
  levels = c("No.incendio", "Incendio") # muy importante
)

plot(roc_rf, col = "#F8766D", lwd = 2, main = "Curva ROC - Random Forest")
auc(roc_rf)  # esto te da el valor del AUC
```

```{r Área bajo la curva, ROC}
auc(roc_rf)
```
